...
const resolveClientReference = $$$config.resolveClientReference;
$$$config.resolveServerReference;
const preloadModule = $$$config.preloadModule;
const requireModule = $$$config.requireModule;
const dispatchHint = $$$config.dispatchHint;
const prepareDestinationForModule = $$$config.prepareDestinationForModule;

const createStringDecoder = $$$config.createStringDecoder;
const readPartialStringChunk = $$$config.readPartialStringChunk;
const readFinalStringChunk = $$$config.readFinalStringChunk;
			...
			if (Symbol.for(name) !== value) {
				throw Error(
					formatProdErrorMessage(
						470, // $FlowFixMe[incompatible-type] `description` might be undefined
						value.description
					)
				);
			}
			...
			default: {
				...
				switch (chunk.status) {
					...
					case CYCLIC:
						...
						chunk.then(
							createModelResolver(parentChunk, parentObject, key, chunk.status === CYCLIC),
...
function resolveErrorProd(response, id, digest) {
	const error = Error(formatProdErrorMessage(441));
...
function resolvePostponeProd(response, id) {
	const error = Error(formatProdErrorMessage(484));
