...
let write = (destination, what) => {
	return 'write' in destination ? destination.write(what) : destination.enqueue(what);
};
function writeToDestination(destination, view) {
	var currentHasCapacity = write(destination, view);
...
function completeWriting(destination) {
	if (currentView && writtenBytes > 0) {
		write(destination, currentView.subarray(0, writtenBytes));
	}
...
function close$1(destination) {
	'end' in destination ? destination.end() : destination.close();
}
...
function closeWithError(destination, error) {
	...
	'destroy' in destination ? destination.destroy(error) : destination.close(error);
}
...
function registerClientReference(proxyImplementation, moduleId, exportName, isLib, root, async = false) {
...
function registerClientReferenceImpl(proxyImplementation, moduleId, isLib, root, async) {
	return Object.defineProperties(proxyImplementation, {
		$$typeof: {
			value: CLIENT_REFERENCE_TAG,
		},
...
function resolveClientReferenceMetadata(config, clientReference) {
	...
	return [modulePath, exportName, clientReference.$$lib, clientReference.$$root, clientReference.$$async];
}
...
var ReactSharedServerInternals = React.__SECRET_SERVER_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED; // $FlowFixMe: It's defined in the one we resolve to.
...
function createRequest(model, bundlerConfig, onError, identifierPrefix, onPostpone) {
	...
	var rootTask = createTask(request, model, abortSet);
...
function serializeThenable(request, thenable) {
	request.pendingChunks++;
	var newTask = createTask(request, null, request.abortableTasks);
	...
	if (typeof type === 'function') {
		...
		switch (type.$$typeof) {
			...
			case REACT_FORWARD_REF_TYPE: {
				...
				return renderModelDestructive(request, task, emptyRoot, '', _result);
			}
...
function outlineModel(request, value) {
	request.pendingChunks++;
	var newTask = createTask(request, value, request.abortableTasks);
...
function renderModel(request, task, parent, key, value) {
	try {
		...
		var wasReactNode =
			typeof model === 'object' &&
			model !== null &&
			(model.$$typeof === REACT_ELEMENT_TYPE || model.$$typeof === REACT_LAZY_TYPE);
		...
		if (typeof x === 'object' && x !== null) {
			...
			if (typeof x.then === 'function') {
				...
				request.pendingChunks++;
				var newTask = createTask(request, task.model, request.abortableTasks);
...
function emitImportChunk(request, id, clientReferenceMetadata) {
	...
	var json = stringify(clientReferenceMetadata.slice(0, 3));
...
function retryTask(request, task) {
	...
	try {
		...
		var json = typeof resolvedModel === 'string' ? stringify(resolvedModel) : stringify(resolvedModel, task.toJSON);
...
function decodeReplyFromBusboy(busboyStream, moduleBasePath) {
	...
	busboyStream.on('error', function (err) {
		reportGlobalError(
			response, // $FlowFixMe[incompatible-call] types Error and mixed are incompatible
			err
		);
	});
...
function renderToReadableStream(model, moduleBasePath, options) {
	var request = createRequest(
		model,
		moduleBasePath,
		options ? options.onError : undefined,
...
var proxyHandlers = {
	get: function (target, name, receiver) {
		switch (name) {
			...
			case 'then':
				...
				if (!target.$$async) {
					...
					// the client.
					var clientReference = registerClientReferenceImpl({}, target.$$id, target.$$lib, target.$$root, true);
					var proxy = new Proxy(clientReference, proxyHandlers); // Treat this as a resolved Promise for React's use()

					target.status = 'fulfilled';
					target.value = proxy;
					var then = (target.then = registerClientReferenceImpl(
		...
		if (!cachedReference) {
			var reference = registerClientReferenceImpl(
				function () {
...
function createClientModuleProxy(moduleId, exportName, isLib, root) {
	var clientReference = registerClientReference(
