...
function writeToDestination(destination, view) {
	const currentHasCapacity = write(destination, view);
...
function registerClientReference(proxyImplementation, id, exportName, isLib, root) {
...
const ReactSharedServerInternals = React.__SECRET_SERVER_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED; // $FlowFixMe: It's defined in the one we resolve to.
...
function retryTask(request, task) {
	...
	try {
		...
		const json = typeof resolvedModel === 'string' ? stringify(resolvedModel) : stringify(resolvedModel, task.toJSON);
	...
	try {
		...
		if (abortableTasks.size > 0) {
			...
			if (enablePostpone && typeof reason === 'object' && reason !== null && reason.$$typeof === REACT_POSTPONE_TYPE) {
				...
				const error =
					reason === undefined ? new Error('The render was aborted by the server without a reason.') : reason;
