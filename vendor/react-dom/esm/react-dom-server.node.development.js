...
var ReactVersion = '18.3.0-experimental-a840ce1cd-20240127';
...
function createRenderState(resumableState, nonce, externalRuntimeConfig, importMap, onHeaders, maxHeadersLength) {
	...
	if (bootstrapScriptContent !== undefined) {
		bootstrapChunks.push(
			...
			stringToChunk(escapeBootstrapAndImportMapScriptContent(bootstrapScriptContent)),
			endInlineScript
		);
	}
	...
	var headers = onHeaders
		? {
				...
				highImagePreloads: '',
				remainingCapacity:
					typeof maxHeadersLength === 'number' ? maxHeadersLength : DEFAULT_HEADERS_CAPACITY_IN_UTF16_CODE_UNITS,
		  }
		: null;
	var renderState = {
		...
		for (var _i = 0; _i < bootstrapModules.length; _i++) {
			...
			if (typeof _scriptConfig === 'string') {
				...
				_props.integrity = _integrity =
					typeof _scriptConfig.integrity === 'string' ? _scriptConfig.integrity : undefined;
...
function pushStyle(target, props, resumableState, renderState, textEmbedded, insertionMode, noscriptTagInScope) {
	{
		if (hasOwnProperty.call(props, 'children')) {
			...
			if (typeof child === 'function' || typeof child === 'symbol' || Array.isArray(child)) {
				var childType =
					typeof child === 'function' ? 'a Function' : typeof child === 'symbol' ? 'a Sybmol' : 'an Array';
...
function pushScript(target, props, resumableState, renderState, textEmbedded, insertionMode, noscriptTagInScope) {
	{
		...
		if (
			...
			!props.src ||
			!(asyncProp && typeof asyncProp !== 'function' && typeof asyncProp !== 'symbol') ||
			props.onLoad ||
			props.onError ||
			insertionMode === SVG_MODE ||
			noscriptTagInScope ||
...
function preconnect(href, crossOrigin) {
	...
	if (typeof href === 'string' && href) {
		var bucket =
			crossOrigin === 'use-credentials' ? 'credentials' : typeof crossOrigin === 'string' ? 'anonymous' : 'default';
		var key = getResourceKey(href);
		...
		switch (as) {
			...
			case 'style': {
				...
				resumableState.styleResources[_key] =
					options && (typeof options.crossOrigin === 'string' || typeof options.integrity === 'string')
		...
		if (hasOwnProperty.call(params, paramName)) {
			...
			if (typeof paramValue === 'string') {
				value +=
					...
					'="' +
					escapeStringForLinkHeaderQuotedParamValueContext(paramValue, paramName) +
					'"';
			}
		}
	}
					...
					for (
						...
						if ((headers.remainingCapacity -= header.length) >= 2) {
							...
							renderState.resets.style[key] =
								typeof props.crossOrigin === 'string' || typeof props.integrity === 'string'
...
function resumeRequest(
	...
	if (typeof postponedState.replaySlots === 'number') {
		...
		var rootSegment = createPendingSegment(
			...
			0,
			null,
			postponedState.rootFormatContext, // Root segments are never embedded in Text on either edge
			false,
			false
		);
		...
		var _rootTask = createRenderTask(
			request,
			null,
			children,
			-1,
	...
	if (typeof node === 'object') {
		...
		throw new Error(
			'Objects are not valid as a React child (found: ' +
				(childString === '[object Object]' ? 'object with keys {' + Object.keys(node).join(', ') + '}' : childString) +
				'). ' +
...
function prepareForStartFlowingIfBeforeAllReady(request) {
	var shellComplete =
		request.trackedPostpones === null // Render Request, we define shell complete by the pending root tasks
...
function resumeRequestImpl(children, postponedState, options) {
	return resumeRequest(
		children,
		postponedState,
		resumeRenderState(postponedState.resumableState, options ? options.nonce : undefined),
		options ? options.onError : undefined,
		options ? options.onAllReady : undefined,
		options ? options.onShellReady : undefined,
		options ? options.onShellError : undefined,
...
export { prerenderToNodeStream, renderToPipeableStream, resumeToPipeableStream, ReactVersion as version };
