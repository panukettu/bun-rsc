...
import * as Scheduler from '@pkxp/react-scheduler';
import { unstable_scheduleCallback, unstable_NormalPriority } from '@pkxp/react-scheduler';
...
function initInput(element, value, defaultValue, checked, defaultChecked, type, name, isHydrating) {
	...
	const initialChecked =
		typeof checkedOrDefault !== 'function' && typeof checkedOrDefault !== 'symbol' && !!checkedOrDefault;
...
function useThenable(thenable) {
	...
	if (
		...
		(workInProgressHook === null ? currentlyRenderingFiber$1.memoizedState === null : workInProgressHook.next === null)
	) {
...
function updateClassInstance(current, workInProgress, ctor, newProps, renderLanes) {
	...
	const shouldUpdate =
		checkHasForceUpdateAfterProcessing() ||
		checkShouldComponentUpdate(workInProgress, ctor, oldProps, newProps, oldState, newState, nextContext) || // TODO: In some cases, we'll end up checking if context has changed twice,
			...
			if (renderedTail === null) {
				...
				if (!didSuspendAlready) {
					...
					const cannotBeSuspended =
						renderHasNotSuspendedYet() && (current === null || (current.flags & DidCapture) === NoFlags$1);
...
function recursivelyTraverseReappearLayoutEffects(finishedRoot, parentFiber, includeWorkInProgressEffects) {
	...
	const childShouldIncludeWorkInProgressEffects =
		includeWorkInProgressEffects && (parentFiber.subtreeFlags & LayoutMask) !== NoFlags$1; // TODO (Offscreen) Check: flags & (RefStatic | LayoutStatic)
	let child = parentFiber.child;
...
function recursivelyTraverseReconnectPassiveEffects(
	...
	// node was reused.
	const childShouldIncludeWorkInProgressEffects =
		includeWorkInProgressEffects && (parentFiber.subtreeFlags & PassiveMask) !== NoFlags$1; // TODO (Offscreen) Check: flags & (RefStatic | LayoutStatic)
	let child = parentFiber.child;
...
function handleThrow(root, thrownValue) {
	...
	if (thrownValue === SuspenseException) {
		...
		const isWakeable =
			thrownValue !== null && typeof thrownValue === 'object' && typeof thrownValue.then === 'function';
...
function replaySuspendedUnitOfWork(unitOfWork) {
	...
	switch (unitOfWork.tag) {
		...
		case FunctionComponent: {
			...
			const resolvedProps =
				unitOfWork.elementType === Component ? unresolvedProps : resolveDefaultProps(Component, unresolvedProps);
			let context;
...
function commitRootImpl(root, recoverableErrors, transitions, renderPriorityLevel, spawnedLane) {
	...
	const subtreeHasEffects =
		(finishedWork.subtreeFlags & (BeforeMutationMask | MutationMask | LayoutMask | PassiveMask)) !== NoFlags$1;
	const rootHasEffect =
		(finishedWork.flags & (BeforeMutationMask | MutationMask | LayoutMask | PassiveMask)) !== NoFlags$1;
...
function scheduleReplayQueueIfNeeded(formReplayingQueue) {
	...
	if (lastScheduledReplayQueue !== formReplayingQueue) {
		...
		unstable_scheduleCallback(unstable_NormalPriority, () => replayUnblockedFormActions(formReplayingQueue));
	}
}
...
const nonDelegatedEvents = new Set(
	['cancel', 'close', 'invalid', 'load', 'scroll', 'scrollend', 'toggle'].concat(mediaEventTypes)
);
			...
			switch (type) {
				...
				case 'link': {
					...
					if (rel === 'stylesheet' && element.hasAttribute('data-precedence')) {
						...
						rel !== anyProps.rel ||
						element.getAttribute('href') !== (anyProps.href == null ? null : anyProps.href) ||
						element.getAttribute('crossorigin') !== (anyProps.crossOrigin == null ? null : anyProps.crossOrigin) ||
						element.getAttribute('title') !== (anyProps.title == null ? null : anyProps.title)
					) {
				...
				case 'script': {
					...
					if (
						...
						element.getAttribute('type') !== (anyProps.type == null ? null : anyProps.type) ||
						element.getAttribute('crossorigin') !== (anyProps.crossOrigin == null ? null : anyProps.crossOrigin)
					) {
...
function canHydrateTextInstance(instance, text, inRootOrSingleton) {
	...
	while (instance.nodeType !== TEXT_NODE) {
		if (instance.nodeType === ELEMENT_NODE && instance.nodeName === 'INPUT' && instance.type === 'hidden');
		else if (!inRootOrSingleton) {
			return null;
		}
		...
		case 'link': {
			...
			if (maybeNodes) {
				...
				for (let i = 0; i < nodes.length; i++) {
					...
					if (
						node.getAttribute('href') !== (props.href == null ? null : props.href) ||
						node.getAttribute('rel') !== (props.rel == null ? null : props.rel) ||
						node.getAttribute('title') !== (props.title == null ? null : props.title) ||
						node.getAttribute('crossorigin') !== (props.crossOrigin == null ? null : props.crossOrigin)
					) {
		...
		case 'meta': {
			...
			if (maybeNodes) {
				...
				for (let i = 0; i < nodes.length; i++) {
					...
					if (
						node.getAttribute('content') !== (props.content == null ? null : '' + props.content) ||
						node.getAttribute('name') !== (props.name == null ? null : props.name) ||
						node.getAttribute('property') !== (props.property == null ? null : props.property) ||
						node.getAttribute('http-equiv') !== (props.httpEquiv == null ? null : props.httpEquiv) ||
						node.getAttribute('charset') !== (props.charSet == null ? null : props.charSet)
					) {
						// mismatch, try the next node;
						continue;
					}
